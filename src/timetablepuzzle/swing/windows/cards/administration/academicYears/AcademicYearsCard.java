...
public class AcademicYearsCard extends JPanel {
	...
	private CustomComboBoxModel<StudentGroup> comboBoxParentStudentGroupModel;
	private JComboBox<StudentGroup> comboBoxParentStudentGroup;
	...
	public AcademicYearsCard(Color backgroundColor) {
		...
		// Parent Student Group combo box
		this.comboBoxParentStudentGroupModel = new CustomComboBoxModel<StudentGroup>();
		RefreshComboBoxParentStudentGroup();
		this.comboBoxParentStudentGroup = new JComboBox<StudentGroup>(this.comboBoxParentStudentGroupModel);
	...
	private void RefreshComboBoxParentStudentGroup() {
		this.comboBoxParentStudentGroupModel.setData(studentGroupDAOService.GetAll());
	}
	...
	private JPanel CreatePropertiesPanel() {
		...
		propertiesPanel.add(
				AddOptionalLabel(CreatePropertyPanel("Third Academic Session ", this.comboBoxThirdAcademicSession)));
	...
	private void CreateAndSaveNew() {
		...
		StudentGroup parentStudentGroup = (StudentGroup) this.comboBoxParentStudentGroup.getSelectedItem();
		...
		if ((name.isEmpty()) || (parentStudentGroup == null) || (firstAcademicSession == null)
				|| (secondAcademicSession == null)) {
			...
			try {
				// Create new entity
				AcademicYear academicYear = new AcademicYear(this.idOfTheAcademicYearToUpdate, name, parentStudentGroup,
						firstAcademicSession, secondAcademicSession);
	...
	private void DeleteSelectedRow() {
		...
		if (selecteRow == -1) {
			...
			try {
				...
				this.notificationLabel
						.setText("An error occured. Please make sure that nothing else depends on this academicYear."
								+ " Check log files for more info.");
				LOGGER.log(Level.SEVERE,
						"Exception occured on deleting academicYear. Please make sure that nothing else depends on this academicYear."
								+ e.toString(),
						e);
			}
		}
